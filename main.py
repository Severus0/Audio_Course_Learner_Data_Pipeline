...
def check_models_exist():
    ...
    if not gguf_files:
        ...
        # Prefer llama-2/3 if available, else pick first
...
def get_configuration(resume=False):
    ...
    language = input("What language is being learned? (e.g., German, Spanish): ").strip()
    if not language: sys.exit(1)
...
def main():
    ...
    if not args.resume:
        if input("Start? (y/n): ").strip().lower() != "y": sys.exit(0)
    ...
    # --- MEMORY CLEANUP ---
    # This is critical for 1080 Ti users. 
    # It ensures Whisper is unloaded before Llama loads.
    force_cleanup()
